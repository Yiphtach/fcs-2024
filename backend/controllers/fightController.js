...
// ✅ Display Fight Setup Page (Choose Universe)
exports.showFightSetup = async (req, res) => {
  try {
    const universes = ['Marvel', 'DC', 'Other'];
...
// ✅ Show Character Selection Based on Universe
exports.showCharacterSelection = async (req, res) => {
  try {
    const { universe } = req.query;
    const characters = await Character.find({ universe });
...
function dynamicFightSimulation(char1, char2) {
  ...
  while (char1Health > 0 && char2Health > 0) {
    ...
    if (char2Health <= 0) {
      ...
      return { winner: char1, loser: char2, log: fightLog, stats: fightStats, char1Health, char2Health };
    }
    ...
    if (char1Health <= 0) {
      ...
      fightStats.totalRounds = round;
      return { winner: char2, loser: char1, log: fightLog, stats: fightStats, char1Health, char2Health };
    }
